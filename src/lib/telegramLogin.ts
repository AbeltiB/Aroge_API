...
export type ResolvePendingLoginResult =
  | { status: 'not-found' | 'denied' }
  | { status: 'verified'; claimId?: string }

export async function resolvePendingLogin(
  token: string,
  identity: TelegramIdentity
): Promise<ResolvePendingLoginResult> {
  ...
  // Aroge Live buyer identity-attach: this token was generated by the
  // seller's app for one specific claim (see POST /telegram/bot/start),
  // shared to the buyer out of band (TikTok DM, WhatsApp, etc.) rather than
  // polled by whoever started it — so the attach has to happen right here,
  // synchronously on verification, not deferred to a poll that may never
  // come from this device.
  if (record.claimId) {
    const buyer = await prisma.user.upsert({
      where: { telegramId: identity.telegramId },
      update: { name: [identity.first_name, identity.last_name].filter(Boolean).join(' ') },
      create: { telegramId: identity.telegramId, name: [identity.first_name, identity.last_name].filter(Boolean).join(' ') },
    })
    await prisma.claim.updateMany({
      where: { id: record.claimId, status: 'CLAIMED' as any, buyerId: null },
      data: { buyerId: buyer.id, telegramUserId: identity.telegramId },
    })
  }
  ...
  return { status: 'verified', claimId: record.claimId }
}
